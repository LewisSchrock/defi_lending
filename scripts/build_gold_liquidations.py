...
def load_silver_data(chain: str = 'ethereum') -> pd.DataFrame:
    """Load silver liquidation data for a single chain."""
...
def load_all_silver_data() -> pd.DataFrame:
    """Load silver liquidation data for all available chains."""
    silver_dir = Path('data/silver/liquidations')
    all_dfs = []
    for chain_dir in sorted(silver_dir.iterdir()):
        if not chain_dir.is_dir():
            continue
        parquet = chain_dir / 'liquidations.parquet'
        if parquet.exists():
            ...
            if 'chain' not in df.columns:
                df['chain'] = chain
            all_dfs.append(df)
            print(f"  {chain:12s}: {len(df):>7,} events, {df['csu'].nunique()} CSUs")
    if not all_dfs:
        raise FileNotFoundError("No silver liquidation data found")
...
def main():
    ...
    # Load silver data from ALL chains
    print("\nLoading silver data from all chains...")
    df = load_all_silver_data()
    print(f"\n  Total: {len(df):,} liquidation events")
    ...
    # Save to all_chains (used by build_gold_panel_all_csus.py)
    output_dir = Path('data/gold/liquidations/all_chains')
