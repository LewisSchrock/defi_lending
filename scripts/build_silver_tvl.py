...
def get_token_symbol_from_market(market: Dict) -> Optional[str]:
    ...
    if symbol == 'NATIVE':
        # Try to infer from chain or return generic
        return 'ETH'  # Most chains use ETH-like native
    return symbol
...
def process_bronze_file(
    ...
    for market in markets:
        ...
        if price is None:
            price = get_token_price(symbol, date_str, web3, chain=chain)
...
def main():
    ...
    for csu_idx, csu_dir in enumerate(sorted(csu_dirs), 1):
        ...
        if not bronze_files:
            print(f"\n  [{csu_idx}/{len(csu_dirs)}] {csu_name}: 0 new files (all {skipped_count} already in silver)")
            continue
