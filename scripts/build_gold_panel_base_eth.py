    ...
    for csu in csus:
        ...
        for jf in json_files:
            try:
                ...
                # FIX: Was bare except — now catches specific exceptions
...
def main():
    ...
    # Save provenance metadata for reproducibility
    metadata = {
        'generated_at': datetime.now(timezone.utc).isoformat(),
        'date_range': {'start': panel['date'].min(), 'end': panel['date'].max()},
        'n_csus': int(panel['csu'].nunique()),
        'n_dates': int(panel['date'].nunique()),
        'n_observations': len(panel),
        'csus': sorted(panel['csu'].unique().tolist()),
        'source_files': {
            'silver_liquidations_base': str(SILVER_LIQ / 'base' / 'liquidations.parquet'),
            'silver_liquidations_ethereum': str(SILVER_LIQ / 'ethereum' / 'liquidations.parquet'),
            'silver_tvl': str(SILVER_TVL),
            'bronze_tvl': str(BRONZE_TVL),
        },
        'coverage': {
            'utilization': f"{util_coverage:.1f}%",
            'volatility': f"{vol_coverage:.1f}%",
        },
        'excluded_csus': sorted(EXCLUDE_CSUS),
    }
    metadata_path = OUTPUT_DIR / "panel_metadata.json"
