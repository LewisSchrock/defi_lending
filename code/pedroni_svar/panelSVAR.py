...
def panelSVAR(input):
    ...
    if logdiff_before_averaging:
        ...
        if len(unit_root_var) > 0:
            input.df[unit_root_var] = input.df.groupby(input.member_col)[unit_root_var].transform(lambda x : np.log(x) - np.log(x).shift(1))

    # Z-score standardize per member before cross-sectional averaging.
    # Puts all variables on equal scale (mean=0, std=1) within each CSU.
    # Required because raw variable scales differ ~1000:1
    # (liquidation log-USD ~3.5 std vs utilization ratio ~0.004 std).
    input.df[variable_cols] = input.df.groupby(input.member_col)[variable_cols].transform(
    ...
    for member, member_df in input.df.groupby(input.member_col):
        ...
        try:
            ...
            print(f"SVAR failed for {member}: {e}")
            continue
