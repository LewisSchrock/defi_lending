...
class KaminoManager {
    ...
    constructor(rpc, recentSlotDurationMs, kaminoLendProgramId, kaminoVaultProgramId) {
        this._rpc = rpc;
        ...
        this._vaultClient = new vault_1.KaminoVaultClient(rpc, this.recentSlotDurationMs, this._kaminoVaultProgramId, this._kaminoLendProgramId);
    }
    getRpc() {
        return this._rpc;
    }
    ...
    async addAssetToMarketIxs(params) {
        ...
        const reserveAccount = await (0, kit_1.generateKeyPairSigner)();
        const createReserveInstructions = await (0, lib_1.createReserveIxs)(this._rpc, params.admin, params.adminLiquiditySource, params.marketAddress, params.assetConfig.mint, params.assetConfig.mintTokenProgram, reserveAccount, this._kaminoLendProgramId);
        const updateReserveInstructions = await this.updateReserveIxs(params.admin, marketWithAddress, reserveAccount.address, params.assetConfig.getReserveConfig(), undefined, false);
        const txnIxs = [];
     ...
     */
    async getSetSharesMetadataIx(authority, vault, tokenName, extraName) {
        const vaultState = await vault.getState();
        return this._vaultClient.getSetSharesMetadataIx(this._rpc, authority, vault.address, vaultState.sharesMint, vaultState.baseVaultAuthority, tokenName, extraName);
    }
    /**
     ...
     */
    async fullRemoveReserveFromVaultIxs(signer, kaminoVault, reserveAddress, reserveState) {
        ...
        const kaminoReserve = await lib_1.KaminoReserve.initializeFromAddress(reserveAddress, connection, this.recentSlotDurationMs, reserveState);
    ...
    async getReserveConfig(reserve) {
        const reserveState = await lib_1.Reserve.fetch(this._rpc, reserve);
        if (!reserveState) {
            throw new Error('Reserve not found');
        }
     ...
     */
    async updateReserveIxs(lendingMarketOwner, marketWithAddress, reserve, config, reserveStateOverride, updateEntireConfig = false) {
        ...
        if (!reserveState || updateEntireConfig) {
            ixs.push(await (0, lib_1.updateEntireReserveConfigIx)(lendingMarketOwner, marketWithAddress.address, reserve, config, this._kaminoLendProgramId));
        }
        else {
            ixs.push(...(await (0, lib_1.parseForChangesReserveConfigAndGetIxs)(marketWithAddress, reserveState, reserve, config, this._kaminoLendProgramId, lendingMarketOwner)));
        }
        return ixs;
    }
    /**
     ...
     */
    async depositToVaultIxs(user, vault, tokenAmount, vaultReservesMap, farmState) {
     ...
     */
    async updateVaultConfigIxs(vault, mode, value, signer, lutIxsSigner, skipLutUpdate = false) {
     ...
     */
    async withdrawFromVaultIxs(user, vault, shareAmount, slot, vaultReservesMap, farmState) {
     ...
     */
    async withdrawPendingFeesIxs(vault, slot, vaultAdminAuthority) {
     ...
     */
    async syncVaultLUTIxs(authority, vault, vaultReserves) {
    ...
    async getAllMarkets(programId = programId_1.PROGRAM_ID) {
        ...
        // Get all reserves
        const allReserveAccounts = (0, lib_1.getAllReserveAccounts)(this.getRpc());
        const reservePairs = [];
        ...
        const allOracleAccounts = await (0, lib_1.getAllOracleAccounts)(this.getRpc(), allReserves);
        ...
        const combinedMarkets = lendingMarketPairs.map(([pubkey, market]) => {
            ...
            if (!reserves) {
                ...
                reservesAndOracles.forEach(([reserve, oracle], index) => {
                    ...
                    const kaminoReserve = lib_1.KaminoReserve.initialize(address, state, oracle, this.getRpc(), this.recentSlotDurationMs);
    ...
    async getAllVaultsForOwner(owner) {
        ...
        const kaminoVaults = await (0, rpc_1.getProgramAccounts)(this._rpc, this._kaminoVaultProgramId, size, filters);
     ...
     */
    async getVaultFarmRewardsAPY(vault, vaultTokenPrice, farmsClient, slot) {
     ...
     */
    async getVaultDelegatedFarmRewardsAPY(vault, vaultTokenPrice, farmsClient, slot) {
    ...
    async getReserveFarmRewardsAPY(reserve, reserveTokenPrice, farmsClient, slot, reserveState) {
        return (0, farmUtils_1.getReserveFarmRewardsAPY)(this._rpc, this.recentSlotDurationMs, reserve, reserveTokenPrice, this._kaminoLendProgramId, farmsClient ? farmsClient : new dist_1.Farms(this._rpc), slot ? slot : await this.getRpc().getSlot().send(), reserveState);
    }
    /**
     ...
     */
    static async getWalletInfo(connection, authority) {
